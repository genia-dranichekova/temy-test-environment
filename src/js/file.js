...
submitButton.addEventListener("click", function () {
    ...
    let txts = document.getElementsByTagName('textarea');
...
function setStatesVisible() {
    ...
    if (document.getElementById("inputCountry").value !== "") {
        // console.log(document.getElementById('inputState').getElementsByTagName("option"));
        xhr.open('GET', 'http://localhost:3000/states/', false);
        xhr.send();
...
function setCitiesVisible() {
    ...
    if (document.getElementById("inputState").value !==  "") {
        ...
        if (xhr.status !== 200) {
            ...
            // select.remove(select.getElementsByTagName("option"));
